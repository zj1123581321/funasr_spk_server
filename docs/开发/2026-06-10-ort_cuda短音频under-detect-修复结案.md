...
- **cluster_merge 的 sherpa extractor 在长单人 turn 上撞 ONNX broadcast
  RuntimeError**（`Where node ... 12288 by 23313`，双 backend 同样撞，
  生产 try/except 容错跳层后该 task 少一层 merge）。短音频矩阵 1spk/2spk
  300s/600s 复现稳定，可作复现入口。
