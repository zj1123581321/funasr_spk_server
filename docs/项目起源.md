...
> ⚠️ **本文档是项目起步时的设计目标，保留作为历史记录。**
>
> 部分需求在后续迭代中已发生变化：
> - 「跨平台兼容性 windows/mac/docker」→ 实际演进为 **mac-only**（依赖 MPS GPU 加速，Docker 不能穿透 Metal）
> - 「无需考虑 GPU 加速，纯 CPU 运行」→ 实际接入了 **MPS GPU 加速**（详见 `docs/开发/gpu加速/`）
> - 「默认 4 个并发任务」→ 实际默认 2 个，可通过 `FUNASR_MAX_CONCURRENT_TASKS` 调整
>
> 当前真相以以下文档为准：
> - **架构现状**：项目根 `CLAUDE.md` ASR 引擎章节（FunASR + Qwen3 双引擎，多 worker pool）
> - **部署**：`docs/部署.md`
> - **客户端协议**：`docs/使用/客户端交互指南.md`
